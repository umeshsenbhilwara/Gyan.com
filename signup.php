...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name) || empty($email) || empty($password) || empty($confirm_password) || !isset($_POST['terms'])) {
...
<body class="bg-bgDark min-h-screen flex items-center justify-center p-4 sm:p-6 lg:p-8">
    <div class="w-full max-w-xs sm:max-w-md lg:max-w-lg bg-white/10 backdrop-blur-md rounded-2xl shadow-xl border border-white/10 p-6 sm:p-8 lg:p-10">
        <div class="flex border-b border-white/20 relative">
            <button id="login-tab" class="flex-1 py-3 sm:py-4 text-center text-white/50 font-medium text-sm sm:text-base lg:text-lg transition-colors duration-300">Login</button>
            <button id="signup-tab" class="flex-1 py-3 sm:py-4 text-center text-white font-medium text-sm sm:text-base lg:text-lg transition-colors duration-300">Sign Up</button>
        ...
        <div class="mt-6 sm:mt-8">
            <form id="signup-form" action="signup.php" method="POST" class="space-y-4 sm:space-y-6">
                ...
                <?php elseif ($success): ?>
                    <div class="text-green-400 text-center text-sm"><?php echo htmlspecialchars($success); ?></div>
                <?php endif; ?>
            ...
            <form id="login-form" action="login.php" method="POST" class="space-y-4 sm:space-y-6 hidden">
                ...
                <div class="flex justify-between items-center text-xs sm:text-sm lg:text-base">
                 ...
                 <!--   <a href="#" class="text-white/70 hover:text-primary hover:underline">Forgot Password?</a> -->
                </div>
                <button type="submit" class="w-full bg-primary text-white font-semibold py-2 sm:py-3 rounded-lg hover:bg-secondary transition-all duration-300 flex items-center justify-center space-x-2">
                    <span>Login</span>
